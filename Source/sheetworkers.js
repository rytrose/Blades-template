...
const getTranslation = (key) =>
  getTranslationByKey(key) || "NO_TRANSLATION_FOUND";
...
Object.keys(data.crew).forEach((crew) => {
  ...
  data.crew[crew].contact = [...Array(data.maxContactsPerCrew).keys()].map(
    (i) => ({
...
Object.keys(data.playbook).forEach((playbook) => {
  ...
  if (!data.friendlessPlaybooks.includes(playbook)) {
    data.playbook[playbook].friend = [
      ...Array(data.maxFriendsPerPlaybook).keys(),
    ]
      .map((i) => ({
  ...
  data.playbook[playbook].ability = data.playbook[playbook].ability.map(
    (name) => ({
...
const mySetAttrs = (attrs, options, callback) => {
  ...
  fillRepeatingSectionFromData = (sectionName, dataList, autogen, callback) => {
    ...
    getSectionIDs(`repeating_${sectionName}`, (idList) => {
      ...
      getAttrs(existingRowAttributes, (v) => {
        ...
        const existingRowNames = idList.map(
  ...
  handleBoxesFill = (name, upToFour) => {
    on(
      `change:${name}1 change:${name}2 change:${name}3 change:${name}4`,
      (event) => {
  ...
  calculateResistance = (name) => {
    getAttrs([...data.actions[name], `setting_resbonus_${name}`], (v) => {
      const total = data.actions[name]
        .map((x) => v[x])
        .reduce((s, c) => s + (String(c) === "0" ? 0 : 1), 0);
      setAttr(name, total);
      setAttr(
        `${name}_formula`,
        buildRollFormula(total + parseInt(v[`setting_resbonus_${name}`]))
      );
    });
  },
  calculateVice = () => {
  ...
  calculateStashFormula = () =>
    getAttrs(["stash"], (v) => {
      setAttr(
        "stash_formula",
        buildRollFormula(Math.floor(parseInt(v.stash) / 10))
      );
    }),
    ...
    getAttrs(sourceAttrs, (v) => {
      ...
      prefixes.forEach((prefix) => {
        const dice =
            ...
            (v[`${prefix}_type`] === "elite" || v[`${prefix}_type`] === "expert"
              ? 1
              : 0),
...
const crewAttributes = [
    ...new Set(
      [].concat(
        ...Object.keys(data.crew).map((x) => Object.keys(data.crew[x].base))
      )
    ),
  ],
  playbookAttributes = [
    ...new Set(
      [].concat(
        ...Object.keys(data.playbook).map((x) =>
          Object.keys(data.playbook[x].base)
        )
      )
    ),
  ],
  ...
  actionsFlat = [].concat(
    ...Object.keys(data.actions).map((x) => data.actions[x])
  ),
  traumaDataFlat = Object.keys(data.traumas).reduce(
    (m, k) => m.concat(data.traumas[k]),
    []
  ),
  autoExpandFields = [
  ...
  getAttrs(
    ...
    (v) => {
      const changedAttributes = (v.changed_attributes || "").split(","),
        sourceName =
          translatedNames[
            (event.sourceAttribute === "crew_type"
              ? v.crew_type
              : v.playbook
            ).toLowerCase()
          ],
        fillBaseData = (inputData, defaultAttrNames) => {
          if (data) {
            const finalSettings = defaultAttrNames
              ...
              .filter(
                (name) =>
                  !spiritPlaybooks.includes(sourceName) ||
      ...
      if (event.sourceAttribute === "crew_type" && sourceName in data.crew) {
        fillRepeatingSectionFromData(
          "contact",
          data.crew[sourceName].contact,
          true
        );
        fillRepeatingSectionFromData(
          "crewability",
          data.crew[sourceName].crewability,
          true
        );
        fillRepeatingSectionFromData(
          "upgrade",
          data.crew[sourceName].upgrade,
          true
        );
      ...
      if (event.sourceAttribute === "playbook" && sourceName in data.playbook) {
        fillRepeatingSectionFromData(
          "friend",
          data.playbook[sourceName].friend,
          true
        );
        fillRepeatingSectionFromData(
          "ability",
          data.playbook[sourceName].ability,
          true
        );
        fillRepeatingSectionFromData(
          "playbookitem",
          data.playbook[sourceName].playbookitem,
          true
        );
...
const fillPlaybookAbility = () => {
  ...
  getAttrs([`${prefix}_name`, `${prefix}_description`], (v) => {
    if (!v[`${prefix}_description`]) {
      const description = playbookAbilityMap.get(
...
const fillCrewAbility = () => {
  ...
  getAttrs([`${prefix}_name`, `${prefix}_description`], (v) => {
    if (!v[`${prefix}_description`]) {
      const description = crewAbilityMap.get(
        (v[`${prefix}_name`] || "").toLowerCase()
      );
      if (description) setAttr(`${prefix}_description`, description);
    }
...
autogenSections.forEach((sectionName) => {
  on(`change:repeating_${sectionName}`, (event) => {
    getAttrs([`repeating_${sectionName}_autogen`], (v) => {
      if (
        v[`repeating_${sectionName}_autogen`] &&
    ...
    if (event.sourceType === "player") {
      getAttrs(["changed_attributes"], (v) => {
        const changedAttributes = [
          ...new Set(v.changed_attributes.split(",")).add(name),
        ]
          .filter((x) => !!x)
          .join(",");
...
Object.keys(data.actions).forEach((attrName) => {
  on(
    [...data.actions[attrName], `setting_resbonus_${attrName}`]
      .map((x) => `change:${x}`)
      .join(" "),
...
/* Calculate trauma */
on(
  "change:setting_traumata_set " +
    traumaDataFlat.map((x) => `change:trauma_${x}`).join(" "),
  (event) => {
    getAttrs(
      ["setting_traumata_set", ...traumaDataFlat.map((x) => `trauma_${x}`)],
      (v) => {
        const traumaType =
          v.setting_traumata_set === "0" ? "normal" : v.setting_traumata_set;
        if (data.traumas[traumaType] && event.sourceType === "player") {
          const newTrauma = data.traumas[traumaType].reduce(
...
autogenSections.forEach((sectionName) => {
  on(`change:generate_${sectionName}`, () => {
    getAttrs(
      ...
      (v) => {
        const dataVar =
            v.sheet_type === "character" ? data.playbook : data.crew,
          genSource = v[`generate_source_${v.sheet_type}`];
        if (genSource in dataVar) {
          emptyFirstRowIfUnnamed(sectionName);
          fillRepeatingSectionFromData(
            sectionName,
            dataVar[genSource][sectionName]
          );
        }
      }
    );
...
on("change:crew_tier", () => {
  getSectionIDs("repeating_cohort", (a) =>
    calculateCohortDice(a.map((id) => `repeating_cohort_${id}`))
  );
});
on(
  "change:char_cohort_quality change:char_cohort_impaired change:setting_show_cohort",
  () => {
    getAttrs(["char_cohort_quality", "char_cohort_impaired"], (v) => {
      const dice =
        (parseInt(v.char_cohort_quality) || 0) -
...
["char_cohort", "cohort1", "repeating_cohort"].forEach((prefix) => {
  const eventString =
    "change:" +
    (prefix === "repeating_cohort" ? `${prefix}:type` : `${prefix}_type`);
  on(eventString, (event) => {
    const verb =
      event.newValue === "expert" ? "^{rolls_their}" : "^{roll_their}";
...
["item", "playbookitem", "upgrade"].forEach((sName) =>
  handleBoxesFill(`repeating_${sName}:check_`)
);
/* Pseudo-radios */
...
on("change:setting_consequence_query sheet:opened", () => {
  getAttrs(["setting_consequence_query"], (v) => {
    const consequenceQuery =
      String(v.setting_consequence_query) === "1"
...
on("change:chat_image", (event) => {
  const match = (event.newValue || "").match(
    /^(https:\/\/s3\.amazonaws\.com\/files\.d20\.io\/images\/.*\.jpg)\?\d+$/
  );
...
on("sheet:opened", () => {
  ...
  const translatedAttrs = {
    ...
    position_query:
      ...
      `${getTranslation("controlled")},position=${getTranslation(
        "controlled"
      )}|` +
      `${getTranslation("desperate")},position=${getTranslation(
        "desperate"
      )}|` +
